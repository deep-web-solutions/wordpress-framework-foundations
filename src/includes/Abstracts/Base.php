...
use DeepWebSolutions\Framework\Utilities\Interfaces\Resources\Traits\Identity;
 ...
 */
abstract class Base implements Identifiable {
	use Identity;
	use Logging;
	use Paths;
	 ...
	 * Base constructor.
	 *
	 * @param   string|null     $id             Unique ID of the class instance. Must be persistent across requests.
	 * @param   string|null     $public_name    The public name of the using class instance. Must be persistent across requests. Mustn't be unique.
	 */
	public function __construct( ?string $id = null, ?string $public_name = null ) {
		$this->set_instance_id( $id ?: hash( 'md5', static::class ) ); // phpcs:ignore
		$this->set_instance_public_name( $public_name ?: static::class ); // phpcs:ignore
	}
