...
namespace DeepWebSolutions\Framework\Utilities\Interfaces\States\Traits\IsDisabled;
 ...
 * @package DeepWebSolutions\WP-Framework\Utilities\Interfaces\States\Traits\IsDisabled
 */
trait IsDisabledTrait {
	...
	protected function maybe_check_is_disabled_traits(): bool {
		if ( false !== array_search( IsDisabledExtensionTrait::class, Misc::class_uses_deep_list( $this ), true ) ) {
			foreach ( Misc::class_uses_deep( $this ) as $trait_name => $deep_used_traits ) {
	...
	protected function maybe_check_is_disabled_local(): bool {
		if ( in_array( IsDisabledLocalTrait::class, Misc::class_uses_deep_list( $this ), true ) && method_exists( $this, 'is_disabled_local' ) ) {
